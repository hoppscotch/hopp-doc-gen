<template>
  <section class='hopp-request'>
    <div>
      ...
      <select :id="`hopp-request__select-${_uid}`" v-model="selectedRequestType">
    ...
    <div
      ...
      'language-javascript': selectedRequestType === 'Fetch' || selectedRequestType === 'JavaScript XHR',
      }">
        <pre><code>{{ requestExample }}</code></pre>
      </div>
    <button :class="{ 'hopp-request__fetching': api.fetching }" @click.prevent="send(`${url}${path}`, method)">
    ...
    <div v-if="api.loaded" class="hopp-request__response">
      <div class="hopp-request__api-message">
        <h4>Response</h4>
        <div class="language-json" v-if="!api.fetching"><pre class="hopp-request__language-json"><code>{{ api.response }}</code></pre></div>
      </div>
    </div>
  </section>
</template>
 ...
 data () {
  ...
  watch: {
    selectedRequestType (reqType) {
      this.requestExample = this.getRequestExample(reqType, this.method, `${this.url}${this.path}`)
    }
  },
  created () {
    this.requestExample = this.getRequestExample(this.requestType, this.method, `${this.url}${this.path}`)
  },
  methods: {
